...
:: get remote version
for /f "delims=" %%a in ('cscript "%TOOLS_PATH%wget.vbs" /s "%REMOTE_PATH%version.txt"') do set r_version=%%a
...
for /f "delims=" %%a in ('cscript "%TOOLS_PATH%wget.vbs" /f "%REMOTE_PATH%filelist.txt" "%LOCAL_PATH%filelist.txt"') do set res=%%a
if not ["%res%"]==["ok"] echo   %res% & goto :eof

:: iterate filelist.txt
for /f "tokens=1,2 delims=: " %%a in ('call %EXEC_TOOL% md5sum.exe -c %LOCAL_PATH%filelist.txt') do (
	:: if checksum is wrong or file does not exists, do update
	if ["%%b"]==["FAILED"] @call :download %%a
)

:: download version.txt 
for /f "delims=" %%a in ('cscript "%TOOLS_PATH%wget.vbs" /f "%REMOTE_PATH%version.txt" "%LOCAL_PATH%version.txt"') do set res=%%a
if not ["%res%"]==["ok"] echo   %res% & goto :eof
...
:download
	...
	for /f "delims=" %%a in ('cscript "%TOOLS_PATH%wget.vbs" /f "%f_remote%" "%f_local%"') do set f_res=%%a
	echo    %f_res%
	echo.
